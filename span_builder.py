...
def build_llm_input_attributes(
    ...
    if user_message is not None:
        ...
        attrs[SA.INPUT_VALUE] = json.dumps(sanitize_value(input_data))
        attrs[SA.INPUT_MIME_TYPE] = "application/json"

        for i, msg in enumerate(messages):
            attrs[f"{SA.LLM_INPUT_MESSAGES}.{i}.{MA.MESSAGE_ROLE}"] = msg["role"]
            attrs[f"{SA.LLM_INPUT_MESSAGES}.{i}.{MA.MESSAGE_CONTENT}"] = msg["content"]
    ...
    if invocation_params:
        attrs[SA.LLM_INVOCATION_PARAMETERS] = json.dumps(invocation_params)
...
def build_llm_output_attributes(
    ...
    if assistant_response:
        attrs[f"{SA.LLM_OUTPUT_MESSAGES}.{msg_idx}.{MA.MESSAGE_ROLE}"] = "assistant"
        attrs[f"{SA.LLM_OUTPUT_MESSAGES}.{msg_idx}.{MA.MESSAGE_CONTENT}"] = sanitize_string(assistant_response)
        msg_idx += 1
...
def build_tool_input_attributes(
    ...
    attrs: Dict[str, Any] = {
        ...
        SA.INPUT_VALUE: json.dumps(sanitize_value(args)),
...
def build_tool_output_attributes(
    ...
    if is_error:
        ...
        if isinstance(parsed, (dict, list)):
            attrs[SA.OUTPUT_VALUE] = json.dumps(parsed)
        else:
            attrs[SA.OUTPUT_VALUE] = json.dumps({"error": str(result)})
    else:
        parsed = _try_parse_json(result)
        if isinstance(parsed, (dict, list)):
            attrs[SA.OUTPUT_VALUE] = json.dumps(sanitize_value(parsed))
        else:
            sanitized = sanitize_value(result)
            if isinstance(sanitized, (dict, list)):
                attrs[SA.OUTPUT_VALUE] = json.dumps(sanitized)
            else:
                attrs[SA.OUTPUT_VALUE] = json.dumps(sanitized)
